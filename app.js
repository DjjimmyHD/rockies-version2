document.addEventListener('DOMContentLoaded', () => {
  return fetch('rockies2019.json')
    .then(response => response.json())
    .then(responseJSON => {
      console.log(responseJSON)
      return responseJSON.reduce((accumulator, currentValue, currentIndex, array) => {
        if (currentValue['LOCATION'] === 'Coors Field - Denver' || currentValue['LOCATION'] === 'Salt River Fields at Talking Stick - Scottsdale') {
          accumulator.home.push(currentValue)
        } else {
          accumulator.away.push(currentValue)
        }
        return accumulator
      }, { home: [], away: [] })
    })
    .then(newSchedule => { console.log(newSchedule) })
})
